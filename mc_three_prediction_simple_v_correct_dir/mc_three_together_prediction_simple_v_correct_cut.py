...
LOG_DIR = save_mother_dir + "/oschulte/Galen/models/log_NN/mc-Scale-three-cut_log_entire_together_train_feature" + str(
...
SAVED_NETWORK = save_mother_dir + "/oschulte/Galen/models/saved_NN/mc-Scale-three-cut_saved_entire_together_networks_feature" + str(
    FEATURE_TYPE) + "_batch" + str(BATCH_SIZE) + "_iterate" + str(
    ITERATE_NUM) + "_lr" + str(learning_rate) + "-" + str(
    REWARD_TYPE) + "_" + MODEL_TYPE + "-" + Random_or_Sequenced + pre_initialize_situation + if_correct_velocity
...
def get_mc_training_batch(state,
    ...
    while current_batch_length < BATCH_SIZE:
        ...
        if t_home == float(0) and t_away == float(0):
            t_end = float(1)
        else:
            t_end = float(0)
        train_number += 1
        if train_number == train_len:
            if FORWARD_REWARD_MODE:
                ...
                batch_return.append((s_t, t_home, t_away, t_end, 1))
        ...
        if FORWARD_REWARD_MODE:
            ...
            batch_return.append((s_t, t_home, t_away, t_end, 0))
        ...
        for dir_game in DIR_GAMES_ALL:
            ...
            if not Random_select:
                while True:
                    ...
                    t_end = [d[3] for d in batch]
                    y_batch = zip(t_home, t_away, t_end)
