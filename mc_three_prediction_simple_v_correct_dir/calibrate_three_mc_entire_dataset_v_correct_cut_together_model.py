...
import unicodedata
import ast
import mc_three_together_prediction_simple_v_correct_cut
import numpy as np
...
MODEL_TYPE = "V3"
BATCH_SIZE = 32
mc_three_together_prediction_simple_v_correct_cut.feature_num = 26
...
SIMPLE_SAVED_NETWORK_PATH = "/cs/oschulte/Galen/models/saved_NN/mc-Scale-three-cut_saved_entire_together_networks_feature{0}_batch{1}_iterate{2}_lr{3}-NEG_REWARD_GAMMA1_{4}-Sequenced{5}{6}".format(
...
if MODEL_TYPE == "V3":
    model_nn = mc_three_together_prediction_simple_v_correct_cut.td_prediction_simple_V3()
# elif MODEL_TYPE == "V4":
...
for calibration_dir_game in os.listdir(calibration_store_dir):
    ...
    data_name = "model_mc_three_cut_together_predict_feature_" + str(
