...
import unicodedata
import ast
import td_prediction_lstm_v_correct_cut_together
import numpy as np
...
SIMPLE_SAVED_NETWORK_PATH = "/cs/oschulte/Galen/models/hybrid_sl_saved_NN/Scale-cut_together_saved_networks_feature{0}_batch{1}_iterate{2}_lr{3}_{4}_v_correct_".format(
...
calibration_store_dir = "/cs/oschulte/Galen/Hockey-data-entire/Hybrid-RNN-Hockey-Training-All-feature{0}-scale" \
                        "-neg_reward_v_correct__length-dynamic/".format(str(FEATURE_TYPE))
data_name = "model_cut_together_predict_Feature{0}_Iter{1}_lr{2}_Batch{3}_MaxLength{4}_Type{5}_v_correct_".format(
    str(FEATURE_TYPE),
    str(ITERATE_NUM),
...
def generate_calibration_data():
    ...
    if MODEL_TYPE == "v3":
        model_nn = td_prediction_lstm_v_correct_cut_together.td_prediction_lstm_V3()
    else:
