...
calibration_store_dir = "/cs/oschulte/Galen/Hockey-data-entire/Hybrid-RNN-Hockey-Training-All-feature{0}-scale-neg_reward_v_correct__length-dynamic".format(
    str(FEATURE_TYPE))

save_csv_name = home_or_away + "td_lstm_v_correct_cut_together_calibration_entire_feature_" + str(
    FEATURE_TYPE) + "_" + MODEL_TYPE + "_Iter" + str(ITERATE_NUM) + "lr" + str(learning_rate_write) + "_batch" + str(
    BATCH_SIZE) + "_sum_2017-7-31.csv"
save_game_csv_name = home_or_away + "td_lstm_v_correct_cut_together_game_record_entire_feature_" + str(
    FEATURE_TYPE) + "_" + MODEL_TYPE + "_Iter" + str(ITERATE_NUM) + "lr" + str(learning_rate_write) + "_batch" + str(
    BATCH_SIZE) + "_sum_2017-7-31.csv"
...
data_together_name = "model_cut_together_predict_Feature{0}_Iter{1}_lr{2}_Batch{3}_MaxLength10_Type{4}_v_correct_".format(
