...
import unicodedata
import ast
import td_three_prediction_RNN_cut_v_correct_together
import numpy as np
...
MODEL_TYPE = "v1"
BATCH_SIZE = 32
td_three_prediction_RNN_cut_v_correct_together.feature_num = 26
...
SIMPLE_SAVED_NETWORK_PATH = "/cs/oschulte/Galen/models/saved_NN/Scale-three-fix_rnn_cut_together_saved_networks_feature{0}_batch{1}_iterate{2}_{3}{4}{5}".format(
    str(FEATURE_TYPE), str(BATCH_SIZE), str(ITERATE_NUM), MODEL_TYPE, pre_initialize_situation, if_correct_velocity)
calibration_store_dir = "/cs/oschulte/Galen/Hockey-data-entire/RNN-Hockey-Training-All-feature{0}-scale-neg_reward_v_correct_length-10".format(
    str(FEATURE_TYPE))
...
if MODEL_TYPE == "v1":
    model_nn = td_three_prediction_RNN_cut_v_correct_together.create_network_RNN()
# elif MODEL_TYPE == "V4":
...
for calibration_dir_game in os.listdir(calibration_store_dir):
    ...
    data_name = "model_three_cut_fix_rnn_together_predict_feature_" + str(
        FEATURE_TYPE) + "_" + MODEL_TYPE + "_Iter" + str(ITERATE_NUM) + "_batch" + str(BATCH_SIZE) + pre_initialize_save + if_correct_velocity
