...
LOG_DIR = save_mother_dir + "/oschulte/Galen/models/log_NN/Scale-three-fix_rnn_cut_together_log_train_feature" + str(
    FEATURE_TYPE) + "_batch" + str(
    BATCH_SIZE) + "_iterate" + str(
    ITERATE_NUM) + "_" + str(MODEL_TYPE) + if_correct_velocity
SAVED_NETWORK = save_mother_dir + "/oschulte/Galen/models/saved_NN/Scale-three-fix_rnn_cut_together_saved_networks_feature" + str(
    FEATURE_TYPE) + "_batch" + str(
    BATCH_SIZE) + "_iterate" + str(
    ITERATE_NUM) + "_" + str(MODEL_TYPE) + if_correct_velocity
DATA_STORE = "/cs/oschulte/Galen/Hockey-data-entire/RNN-Hockey-Training-All-feature{0}-scale-neg_reward{2}length-{1}".format(
    str(FEATURE_TYPE), str(TRACE_LENGTH), if_correct_velocity)
