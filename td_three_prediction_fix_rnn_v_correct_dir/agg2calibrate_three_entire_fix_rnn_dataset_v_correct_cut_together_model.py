...
save_csv_name = home_or_away + "fix_rnn_td_three_cut_together_calibration_entire_feature_" + str(
...
save_game_csv_name = home_or_away + "fix_rnn_td_three_cut_together_game_record_entire_feature_" + str(
    FEATURE_TYPE) + "_" + MODEL_TYPE + "_Iter" + str(ITERATE_NUM) + "_batch" + str(
    BATCH_SIZE) + pre_initialize_save + if_correct_velocity + "sum_2017-8-05.csv"

data_together_name =  "model_three_cut_fix_rnn_together_predict_feature_" + str(
    FEATURE_TYPE) + "_" + MODEL_TYPE + "_Iter" + str(ITERATE_NUM) + "_batch" + str(
    BATCH_SIZE) + pre_initialize_save + if_correct_velocity + ".mat"

data_together_load_name = "model_three_cut_fix_rnn_together_predict_feature_" + str(
    FEATURE_TYPE) + "_" + MODEL_TYPE + "_Iter" + str(ITERATE_NUM) + "_batch" + str(
...
def agg2calibrate_model(check_target):
    ...
    for calibration_dir_game in os.listdir(calibration_store_dir):
        ...
        model_predict_together = ((sio.loadmat(model_predict_together_name))[data_together_load_name]).tolist()
