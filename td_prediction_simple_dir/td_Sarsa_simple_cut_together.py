...
if Scale:
    ...
    LOG_DIR = save_mother_dir + "/oschulte/Galen/models/Sarsa_log_NN/Sarsa_Scale-cut_log_entire_" + str(
    ...
    SAVED_NETWORK = save_mother_dir + "/oschulte/Galen/models/Sarsa_saved_NN/Sarsa_Scale-cut_saved_entire_" + str(
    ...
    LOG_DIR = save_mother_dir + "/oschulte/Galen/models/Sarsa_log_NN/Sarsa_cut_log_entire_" + str(
        Home_model_or_away_model) + "_train_feature" + str(
        FEATURE_TYPE) + "_batch" + str(BATCH_SIZE) + "_iterate" + str(
        ITERATE_NUM) + "_lr" + str(
        learning_rate) + "-" + str(
        REWARD_TYPE) + "_" + MODEL_TYPE + "-" + Random_or_Sequenced + pre_initialize_situation
    SAVED_NETWORK = save_mother_dir + "/oschulte/Galen/models/Sarsa_saved_NN/Sarsa_cut_saved_entire_" + str(
        Home_model_or_away_model) + "_networks_feature" + str(
        FEATURE_TYPE) + "_batch" + str(BATCH_SIZE) + "_iterate" + str(
        ITERATE_NUM) + "_lr" + str(
        learning_rate) + "-" + str(
...
def Sarsa_get_cut_training_batch(s_t0, state, reward, train_number, train_len):
    """
    ...
    while current_batch_length < BATCH_SIZE:
        ...
        if train_number + 1 == train_len:
            if FORWARD_REWARD_MODE:
                ...
                if r_t1 == float(0):
                    ...
                    batch_return.append((s_t0, r_t1_combine, s_t1, 1, 0, 0))
                elif r_t1 == float(-1):
                    print r_t1
                    r_t1_combine = [float(0), float(1)]
                    batch_return.append((s_t0, r_t1_combine, s_t1, 0, 0, 1))
                    batch_return.append((s_t1, r_t1_combine, s_t1, 1, 0, 0))
                elif r_t1 == float(1):
                    print r_t1
        ...
        if FORWARD_REWARD_MODE:
            ...
            if r_t1 == float(0):
                r_t1_combine = [float(0), float(0)]
                batch_return.append((s_t0, r_t1_combine, s_t1, 0, 0, 0))

            elif r_t1 == float(-1):
                print r_t1
                r_t1_combine = [float(0), float(1)]
                batch_return.append((s_t0, r_t1_combine, s_t1, 0, 0, 1))
                batch_return.append((s_t1, r_t1_combine, s_t1, 0, 1, 0))
                s_t0 = s_t1
                break
            elif r_t1 == float(1):
                print r_t1
            ...
            if not Random_select:
                while True:
                    try:
                        ...
                        s_tl, batch_return, train_number = Sarsa_get_cut_training_batch(s_t0, state,
                                                                                        reward,
                                                                                        train_number,
                                                                                        train_len)
                    except:
                    ...
                    r_t1_home_batch = [d[1] for d in batch]
                    s_t1_home_batch = [d[2] for d in batch]
                    s_t_batch = s_t_home_batch
                    r_t_batch = r_t1_home_batch
