...
if Scale:
    model_data_store_dir = "/cs/oschulte/Galen/Hockey-data-entire/Hockey-Training-All-feature{0}-scale-neg_reward".format(
        str(FEATURE_TYPE))
    state_model_data_store_dir = "/cs/oschulte/Galen/Hockey-data-entire/State-Hockey-Training-All-feature{0}-scale-neg_reward".format(
        str(FEATURE_TYPE))
else:
    model_data_store_dir = "/cs/oschulte/Galen/Hockey-data-entire/Hockey-Training-All-feature{0}-neg_reward".format(
        str(FEATURE_TYPE))
    state_model_data_store_dir = "/cs/oschulte/Galen/Hockey-data-entire/State-Hockey-Training-All-feature{0}-scale-neg_reward".format(
        str(FEATURE_TYPE))

player_info_dir = "/cs/oschulte/Galen/PycharmProjects/Sport-Analytic-NN/td_prediction_simple_dir/player_ranking_dir/players_2015_2016.csv"

skater_info_dir = "/cs/oschulte/Galen/PycharmProjects/Sport-Analytic-NN/td_prediction_simple_dir/player_ranking_dir/skater_stats_2015_2016_original.csv"
...
state_data_name = "model_state_cut_together_predict_fea{0}_{1}_Iter{2}_lr{3}_batch{4}".format(str(FEATURE_TYPE), MODEL_TYPE,
                                                                                       str(ITERATE_NUM),
                                                                                       str(6), str(BATCH_SIZE)) # TODO: we don't have state model fot lr5
    ...
    for calibration_dir_game in os.listdir(model_data_store_dir):
        ...
        for player_Index in range(0, len(playerIds)):
            ...
            if player_value is None:
                if ishome:
                    PLAYER_ID_DICT_ALL.update({playerId: {"value": model_value[0] - model_value[1], "state value": model_state_value[0] - model_state_value[1]}})
                else:
                    PLAYER_ID_DICT_ALL.update({playerId: {"value": model_value[1] - model_value[0], "state value": model_state_value[1] - model_state_value[0]}})
            else:
                if ishome:
                    player_value_number = player_value.get("value") + model_value[0] - model_value[1]
                    player_state_value_number = player_value.get("state value") + model_state_value[0] - model_state_value[1]
                else:
                    player_value_number = player_value.get("value") + model_value[1] - model_value[0]
                    player_state_value_number = player_value.get("state value") + model_state_value[1] - model_state_value[0]
                PLAYER_ID_DICT_ALL.update({playerId: {"value": player_value_number, "state value": player_state_value_number}})
                # break
...
if __name__ == '__main__':
    ...
    write_csv("./player_ranking_dir/player_ranking_test.csv", player_value_dict_list)
