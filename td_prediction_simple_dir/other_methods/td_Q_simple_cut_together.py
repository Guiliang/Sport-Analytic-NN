...
if Scale:
    DATA_STORE = "/cs/oschulte/Galen/Hockey-data-entire/State-Hockey-Training-All-feature5-scale-" + "neg_reward"

    LOG_DIR = save_mother_dir + "/oschulte/Galen/models/log_NN/Scale-Q-cut_log_entire_" + str(
    ...
    SAVED_NETWORK = save_mother_dir + "/oschulte/Galen/models/saved_NN/Scale-Q-cut_saved_entire_" + str(
    ...
    DATA_STORE = "/cs/oschulte/Galen/Hockey-data-entire/State-Hockey-Training-All-feature5-" + "neg_reward"

    LOG_DIR = save_mother_dir + "/oschulte/Galen/models/log_NN/Q-cut_log_entire_" + str(
        Home_model_or_away_model) + "_train_feature" + str(
        FEATURE_TYPE) + "_batch" + str(BATCH_SIZE) + "_iterate" + str(
        ITERATE_NUM) + "_lr" + str(
        learning_rate) + "-" + str(
        REWARD_TYPE) + "_" + MODEL_TYPE + "-" + Random_or_Sequenced + pre_initialize_situation
    SAVED_NETWORK = save_mother_dir + "/oschulte/Galen/models/saved_NN/Q-cut_saved_entire_" + str(
        Home_model_or_away_model) + "_networks_feature" + str(
        FEATURE_TYPE) + "_batch" + str(BATCH_SIZE) + "_iterate" + str(
        ITERATE_NUM) + "_lr" + str(
        learning_rate) + "-" + str(
...
class td_Q_simple_V3(object):
    def __init__(self):
        ...
        self.action = tf.placeholder("float", [None, ACTION_NUMBER * 2])

        with tf.name_scope("cost"):
            self.read_out_action = tf.multiply(self.read_out, self.action)
            read_out_action_reshape = tf.reshape(self.read_out_action, [-1])
            self.y_reshape = tf.reshape(self.y, [-1])
            zero = tf.constant(0, dtype=tf.float32)
            where = tf.not_equal(read_out_action_reshape, zero)
            self.nonezero_indices = tf.where(where)
            self.read_out_action_nonzero = tf.gather(read_out_action_reshape, self.nonezero_indices)
            self.cost = tf.reduce_mean(tf.square(self.y_reshape - self.read_out_action_nonzero))
            self.diff_v = tf.reduce_mean(tf.abs(self.y_reshape - self.read_out_action_nonzero))

            # self.readout_square = tf.square(
            #     tf.reduce_mean(self.y, reduction_indices=1) - tf.reduce_mean(self.read_out_action, reduction_indices=1))
            # self.cost = tf.reduce_mean(self.readout_square)
            # self.readout_diff = tf.abs(
            #     tf.reduce_mean(self.y, reduction_indices=1) - tf.reduce_mean(self.read_out_action, reduction_indices=1))
...
def get_cut_training_batch(s_t0, state, action, reward, train_number, train_len):
    """
    ...
    while current_batch_length < BATCH_SIZE:
        ...
        a_t1 = action[train_number]
        # r_t1 = reward[train_number]
        r_t0 = reward[train_number - 1]
        a_t0 = action[train_number - 1]
        r_t1 = reward[train_number]
        train_number += 1
        if train_number + 1 == train_len:
            if FORWARD_REWARD_MODE:
                ...
                if r_t0 == float(0):
                    ...
                    batch_return.append((s_t0, a_t0, r_t0_combine, s_t1, a_t1, 0, 0))
                    ...
                    batch_return.append((s_t1, a_t1, r_t1_combine, s_t1, a_t1, 1, 0))
                elif r_t0 == float(-1):
        ...
        if FORWARD_REWARD_MODE:
            ...
            if r_t0 == float(0):
                ...
                r_t0_combine = [float(0), float(1)]
                batch_return.append((s_t0, a_t0, r_t0_combine, s_t1, a_t1, 0, 1))
                s_t0 = s_t1
                break
            elif r_t0 == float(1):
                r_t0_combine = [float(1), float(0)]
...
def train_network(sess, model, print_parameters=False):
    ...
    while True:
        ...
        for dir_game in DIR_GAMES_ALL:
            ...
            state = state['state']
            action = sio.loadmat(DATA_STORE + "/" + dir_game + "/" + action_name)
            ...
            if not Random_select:
                while True:
                    try:
                        ...
                        s_tl, batch_return, train_number = get_cut_training_batch(s_t0, state, action,
                                                                                  reward,
                                                                                  train_number,
                                                                                  train_len)
                    except:
                    ...
                    a_t_batch = [np.tile(d[1], 2) for d in batch]
                    r_t_batch = [d[2] for d in batch]
                    s_t1_batch = [d[3] for d in batch]
                    a_t1_batch = [np.tile(d[4], 2) for d in batch]
                    ...
                    if MODEL_TYPE == "V5":
                        ...
                        [nonezero_indices, diff_v, cost_out, summary_train, _] = sess.run(
                            [model.nonezero_indices, model.diff_v, model.cost, merge, model.train_step],
                            feed_dict={model.y: y_batch, model.x: s_t_batch, model.action: a_t_batch})
                    ...
                    if terminal or ((train_number - 1) / BATCH_SIZE) % 5 == 1:
                        ...
                        v_home_avg = sum(readout_t1_batch[:, 0:ACTION_NUMBER]) / len(readout_t1_batch[:, 0:ACTION_NUMBER])
                        v_away_avg = sum(readout_t1_batch[:, ACTION_NUMBER:ACTION_NUMBER*2]) / len(readout_t1_batch[:, ACTION_NUMBER:ACTION_NUMBER*2])
