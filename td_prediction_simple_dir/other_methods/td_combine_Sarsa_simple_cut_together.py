...
if Scale:
    ...
    LOG_DIR = save_mother_dir + "/oschulte/Galen/models/log_NN/combine_Sarsa_Scale-cut_log_entire_" + str(
    ...
    SAVED_NETWORK = save_mother_dir + "/oschulte/Galen/models/saved_NN/combine_Sarsa_Scale-cut_saved_entire_" + str(
    ...
    LOG_DIR = save_mother_dir + "/oschulte/Galen/models/log_NN/combine_Sarsa_cut_log_entire_" + str(
        Home_model_or_away_model) + "_train_feature" + str(
        FEATURE_TYPE) + "_batch" + str(BATCH_SIZE) + "_iterate" + str(
        ITERATE_NUM) + "_lr" + str(
        learning_rate) + "-" + str(
        REWARD_TYPE) + "_" + MODEL_TYPE + "-" + Random_or_Sequenced + pre_initialize_situation
    SAVED_NETWORK = save_mother_dir + "/oschulte/Galen/models/saved_NN/combine_Sarsa_cut_saved_entire_" + str(
        Home_model_or_away_model) + "_networks_feature" + str(
        FEATURE_TYPE) + "_batch" + str(BATCH_SIZE) + "_iterate" + str(
        ITERATE_NUM) + "_lr" + str(
        learning_rate) + "-" + str(
    ...
    while current_batch_length < BATCH_SIZE:
        ...
        if train_number + 1 == train_len:
            if FORWARD_REWARD_MODE:
                ...
                if r_t0 == float(0):
                    ...
                    batch_return.append((s_t0, r_t0_combine, r_t1_combine, s_t1, 0, 0))
                    batch_return.append((s_t1, r_t1_combine, r_t1_combine, s_t1, 1, 0))
                elif r_t0 == float(-1):
        ...
        if FORWARD_REWARD_MODE:
            ...
            if r_t0 == float(0):
                ...
                r_t0_combine = [float(0), float(1)]
                r_t1_combine = [float(0),
                                float(1)]  # TODO:I don't think it's right, it make shot becomes the largest value
                batch_return.append((s_t0, r_t0_combine, r_t1_combine, s_t1, 0, 1))
                s_t0 = s_t1
                break
            elif r_t0 == float(1):
                r_t0_combine = [float(1), float(0)]
        ...
        for dir_game in DIR_GAMES_ALL:
            ...
            if not Random_select:
                while True:
                    ...
                    r_t1_home_batch = [d[2] for d in batch]
                    s_t1_home_batch = [d[3] for d in batch]
                    s_t_batch = s_t_home_batch
                    r_t_batch = r_t_home_batch
                    r1_t_batch = r_t1_home_batch
                    ...
                    for i in range(0, len(batch)):
                        ...
                        if terminal or cut:
                            y_v_home = float((r_t_batch[i])[0])
                            y_v_away = float((r_t_batch[i])[1])
                            y_q_home = float((r1_t_batch[i])[0])
                            y_q_away = float((r1_t_batch[i])[1])
                            y_batch.append([y_v_home, y_v_away, y_q_home, y_q_away])
                            break
                        else:
                            y_v_home = float((r_t_batch[i])[0]) + GAMMA * ((readout_t1_batch[i]).tolist())[0]
                            y_v_away = float((r_t_batch[i])[1]) + GAMMA * ((readout_t1_batch[i]).tolist())[1]
                            y_q_home = float((r1_t_batch[i])[0]) + GAMMA * ((readout_t1_batch[i]).tolist())[2]
                            y_q_away = float((r1_t_batch[i])[1]) + GAMMA * ((readout_t1_batch[i]).tolist())[3]
                            y_batch.append([y_v_home, y_v_away, y_q_home, y_q_away])
