...
if Scale:
    ...
    LOG_DIR = "/cs/oschulte/Galen/models/log_NN/Scale-cut_log_entire_" + str(
    ...
    SAVED_NETWORK = "/cs/oschulte/Galen/models/saved_NN/Scale-cut_saved_entire_" + str(
    ...
    LOG_DIR = "/cs/oschulte/Galen/models/log_NN/cut_log_entire_" + str(
        Home_model_or_away_model) + "_train_feature" + str(
        FEATURE_TYPE) + "_batch" + str(BATCH_SIZE) + "_iterate" + str(
        ITERATE_NUM) + "-" + str(REWARD_TYPE) + "_" + MODEL_TYPE + "-" + Random_or_Sequenced + pre_initialize_situation
...
def parameter_setting(sess, model):
    ...
    while diff_v > 0.001:
        ...
        [diff_v, cost_out, _] = sess.run(
            ...
            feed_dict={model.y: np.asarray(reward_average), model.x: np.asarray(state_average)})
...
def train_network(sess, model, print_parameters=False):
    ...
    for dir_game in DIR_GAMES_ALL:
        ...
        reward_game = sio.loadmat(DATA_STORE + "/" + dir_game + "/" + reward_name)
        try:
            reward_game = (reward_game['reward'][0]).tolist()
        except:
            print "\n" + dir_game
            continue
        reward_count = sum(reward_game)
        state_game = sio.loadmat(DATA_STORE + "/" + dir_game + "/" + state_name)
        state_game = state_game['state']
        print ("\n load file" + str(dir_game) + " success")
        print ("reward number" + str(reward_count))
        if len(state_game) != len(reward_game):
            raise Exception('state length does not equal to reward length')

        state_all.append(state_game)
        reward_all.append(reward_game)
    ...
    while True:
        ...
        for game_index in range(0, len(state_all)):
