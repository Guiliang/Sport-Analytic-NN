...
MODEL_TYPE = "V3"
BATCH_SIZE = 32
td_combine_Sarsa_simple_cut_together.feature_num = 26
...
if Scale:
    SIMPLE_SAVED_NETWORK_PATH = "/cs/oschulte/Galen/models/saved_NN/combine_Sarsa_Scale-cut_saved_entire_together_networks_feature{0}_batch{1}_iterate{2}_lr{3}-NEG_REWARD_GAMMA1_{4}-Sequenced{5}".format(
    ...
    SIMPLE_SAVED_NETWORK_PATH = "/cs/oschulte/Galen/models/saved_NN/combine_Sarsa_cut_saved_entire_together_networks_feature{0}_batch{1}_iterate{2}_lr{3}-NEG_REWARD_GAMMA1_{4}-Sequenced{5}".format(
        str(FEATURE_TYPE), str(BATCH_SIZE), str(ITERATE_NUM), str(learning_rate), MODEL_TYPE, pre_initialize_situation)
...
if MODEL_TYPE == "V3":
    model_nn = td_combine_Sarsa_simple_cut_together.td_prediction_simple_V3()
# elif MODEL_TYPE == "V4":
...
for calibration_dir_game in os.listdir(calibration_store_dir):
    ...
    data_name = "model_combine_Sarsa_cut_together_predict_feature_" + str(
