...
LOG_DIR = "./log_NN/log_state_"+str(Home_model_or_away_model)+"_train_feature" + str(FEATURE_TYPE) + "_batch" + str(BATCH_SIZE) + "_iterate" + str(
    ITERATE_NUM) + "-" + str(REWARD_TYPE) + TRAIN_or_TEST + "-" + Random_or_Sequenced
SAVED_NETWORK = "./saved_state_NN/saved_"+str(Home_model_or_away_model)+"_networks_feature" + str(FEATURE_TYPE) + "_batch" + str(BATCH_SIZE) + "_iterate" + str(
    ITERATE_NUM) + "-" + str(REWARD_TYPE) + TRAIN_or_TEST + "-" + Random_or_Sequenced
            ...
            if not Random_select:
                while True:
                    try:
                        ...
                        s_tl, batch_home_return, batch_away_return, train_number = get_training_batch(s_t0, state, reward, train_number, train_len)
                    except:
