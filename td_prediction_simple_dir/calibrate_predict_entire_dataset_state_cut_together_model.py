...
MODEL_TYPE = "V3"
BATCH_SIZE = 32
td_state_prediction_simple_cut_together.feature_num = 12
learning_rate = 1e-6
...
if Scale:
    ...
    SIMPLE_SAVED_NETWORK_PATH = "/cs/oschulte/Galen/models/saved_NN/state-cut_saved_entire_together_networks_feature{0}_batch{1}_iterate{2}_lr{3}-NEG_REWARD_GAMMA1_{4}-Sequenced{5}".format(
        str(FEATURE_TYPE), str(BATCH_SIZE), str(ITERATE_NUM), str(learning_rate), MODEL_TYPE, pre_initialize_situation)
...
if MODEL_TYPE == "V3":
    model_nn = td_state_prediction_simple_cut_together.td_prediction_simple_V3()
# elif MODEL_TYPE == "V4":
...
for calibration_dir_game in os.listdir(calibration_store_dir):
    ...
    data_name = "model_state_cut_together_predict_fea" + str(
