...
if __name__ == '__main__':
    ...
    for min_sample_leaf in [5, 10, 30, 40, 50]:
        if test_flag:
            ...
            cv_results_record_name = 'cv_running_record_leaf{0}.txt'.format(str(min_sample_leaf))
        ...
        if not read_model:
            ...
            with open('./dt_record/' + cv_results_record_name, 'w') as f:
                f.close()
            all_input_list, all_impact_list = TR.gather_impact_data(apply_cv, all_data_dir)
            for i in range(0, 5):
                ...
                with open('./dt_record/' + cv_results_record_name, 'a') as f:
                    f.write(
                        'The {4} running, mae:{0}, var_mae:{1}, mse:{2}, var_mse:{3}\n'.format(str(mae), str(var_mae),
                                                                                               str(mse), str(var_mse),
                                                                                               str(i)))
                with open(
                        '/cs/oschulte/Galen/soccer-models/dt_datas/dt_training_data_{0}_{1}.pkl'.format(action_selected,
                                                                                                        str(i)),
                        'wb') as f:
                    pickle.dump([training_input_list, training_impact_list], f)
                with open(
                        '/cs/oschulte/Galen/soccer-models/dt_datas/dt_testing_data_{0}_{1}.pkl'.format(action_selected,
                                                                                                       str(i)),
                        'wb') as f:
