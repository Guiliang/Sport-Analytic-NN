...
if MODEL_TYPE == "v3":
    nn = td_prediction_state_lstm_cut_together.td_prediction_lstm_V3()
else:
...
if ISHOME:
    SIMULATION_DATA_PATH = "/cs/oschulte/Galen/Hockey-data-entire/Simulation-data-feature{0}/entire_state_spatial_simulation/State_LSTM_Home_entire_state_spatial_simulation-{1}-feature{0}.mat".format(
...
def nn_simulation(SIMULATION_DATA_PATH, SIMPLE_SAVED_NETWORK_PATH, nn_image_save_dir, nn_half_image_save_dir):
    ...
    if DRAW_TARGET == "V_home":
        value_spatial = value_spatial_home
        vmin_set = None
        vmax_set = None
    elif DRAW_TARGET == "V_away":
    ...
    try:
        plt.title("{1} history length:{0} with DT-LSTM".format(str(len(HIS_ACTION_TYPE)), DRAW_TARGET),
                  fontsize=20)
    except:
        plt.title("{1} history length:{0} with DT-LSTM".format(str(0), DRAW_TARGET), fontsize=20)
    ...
    try:
        plt.title("{1} history length:{0} \n with DT-LSTM on right rink".format(str(len(HIS_ACTION_TYPE)),
                                                                                DRAW_TARGET),
                  fontsize=30)
    except:
        plt.title("{1} history length:{0} \n with DT-LSTM on right rink".format(str(0), DRAW_TARGET),
                  fontsize=30)
