...
def get_training_batch(s_t0, state_input, reward, train_number, train_len, state_trace_length):
    ...
    while current_batch_length < BATCH_SIZE:
        ...
        if train_number + 1 == train_len:
            trace_length_index = s_length_t1 - 1
            batch_return.append((s_t0, s_t1, np.asarray([s_reward[trace_length_index]]), s_length_t0, s_length_t1, 1))
            break
        trace_length_index = s_length_t1 - 1
        batch_return.append((s_t0, s_t1, np.asarray([s_reward[trace_length_index]]), s_length_t0, s_length_t1, 0))
...
def train_network(sess, model, print_parameters=False):
    ...
    while True:
        ...
        for dir_game in DIR_GAMES_ALL:
            ...
            while True:
                # try:
                batch, train_number = get_training_batch(s_t0, state_input, reward, train_number,
