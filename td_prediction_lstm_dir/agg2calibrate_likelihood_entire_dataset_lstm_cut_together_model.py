...
save_csv_name = home_or_away + "td_likelihood_lstm_cut_together_calibration_entire_feature_" + str(
    FEATURE_TYPE) + "_" + MODEL_TYPE + "_Iter" + str(ITERATE_NUM) + "lr" + str(learning_rate_write) + "_batch" + str(
    BATCH_SIZE) + "_sum_2017-8-02.csv"
save_game_csv_name = home_or_away + "td_likelihood_lstm_cut_together_game_record_entire_feature_" + str(
    FEATURE_TYPE) + "_" + MODEL_TYPE + "_Iter" + str(ITERATE_NUM) + "lr" + str(learning_rate_write) + "_batch" + str(
    BATCH_SIZE) + "_sum_2017-8-02.csv"
...
def generate_episode_store(calibrate_names):
    episode_num = 0
    episode_cut_dict = {}
    episode_record_likelihood_home = {episode_num: []}
    episode_record_likelihood_away = {episode_num: []}
    episode_record_likelihood_all = {episode_num: []}
    for calibrate_name_index in range(0, len(calibrate_names)):
        ...
        if action == "goal":
            ...
            episode_num += 1
            episode_record_likelihood_home.update({episode_num: []})
            episode_record_likelihood_away.update({episode_num: []})
            episode_record_likelihood_all.update({episode_num: []})

    if not episode_cut_dict.get(len(calibrate_names) - 1):
        episode_cut_dict.update({len(calibrate_names) - 1: episode_num})

    return episode_cut_dict, episode_record_likelihood_home, episode_record_likelihood_away, episode_record_likelihood_all


def agg2calibrate_likelihood_model(check_target):
    likelihood_home_value_record = []
    likelihood_away_value_record = []
    likelihood_all_value_record = []
    ...
    for calibration_dir_game in os.listdir(calibration_store_dir):
        ...
        episode_cut_dict, episode_record_likelihood_home, episode_record_likelihood_away, episode_record_likelihood_all = generate_episode_store(
                ...
                try:
                    ...
                    for index_episode_cut_index in range(0, len(index_episode_cuts)):
                        if calibrate_name_index <= index_episode_cuts[index_episode_cut_index]:
                            ...
                            if summation_cut_goal_home[calibrate_name_index] == float(1):
                                (episode_record_likelihood_home.get(episode_index)).append(
                                    math.log(model_predict_together[calibrate_name_index][0]))
                                (episode_record_likelihood_all.get(episode_index)).append(
                                    math.log(model_predict_together[calibrate_name_index][0]))
                            elif summation_cut_goal_away[calibrate_name_index] == float(-1):
                                (episode_record_likelihood_away.get(episode_index)).append(
                                    math.log(model_predict_together[calibrate_name_index][1]))
                                (episode_record_likelihood_all.get(episode_index)).append(
                                    math.log(model_predict_together[calibrate_name_index][1]))
                            else:
                                continue
        ...
        for episode_record_likelihood_all_index in range(0, len(episode_record_likelihood_all)):
            try:
                likelihood_all_value_record.append(
                    sum(episode_record_likelihood_all[episode_record_likelihood_all_index]) / len(
                        episode_record_likelihood_all[episode_record_likelihood_all_index]))
            except:
                # calibration_home_value_record.append(float(0))
                continue

        for episode_record_likelihood_away_index in range(0, len(episode_record_likelihood_away)):
            try:
                likelihood_away_value_record.append(
                    sum(episode_record_likelihood_away[episode_record_likelihood_away_index]) / len(
                        episode_record_likelihood_away[episode_record_likelihood_away_index]))
            except:
                # model_predict_away_value_record.append(float(0))
                continue

        for episode_record_likelihood_home_index in range(0, len(episode_record_likelihood_home)):
            try:
                likelihood_home_value_record.append(
                    sum(episode_record_likelihood_home[episode_record_likelihood_home_index]) / len(
                        episode_record_likelihood_home[episode_record_likelihood_home_index]))
            except:
    ...
    try:
        likelihood_home_value_record_average = float(sum(likelihood_home_value_record)) / len(
            likelihood_home_value_record)
    except:
        likelihood_home_value_record_average = 0

    try:
        likelihood_away_value_record_average = float(sum(likelihood_away_value_record)) / len(
            likelihood_away_value_record)
    except:
        likelihood_away_value_record_average = 0

    try:
        likelihood_all_value_record_average = float(sum(likelihood_all_value_record)) / len(
            likelihood_all_value_record)
    except:
        likelihood_all_value_record_average = 0

    print "likelihood_home_value_record_average with " + str(check_target) + " is " + str(
        likelihood_home_value_record_average)
    print "likelihood_away_value_record_average with " + str(check_target) + " is " + str(
        likelihood_away_value_record_average)
    print "likelihood_all_value_record_average with " + str(check_target) + " is " + str(
        likelihood_all_value_record_average)

    return likelihood_home_value_record_average, likelihood_away_value_record_average, likelihood_all_value_record_average, found_game_name_record
    ...
    for goal_diff in [-3, -2, -1, 0, 1, 2, 3]:
        ...
        for manpower in [-1, 0, 1]:
            ...
            for period in [1, 2, 3]:
                ...
                likelihood_home_value_record_average, likelihood_away_value_record_average, likelihood_all_value_record_average, found_game_list = agg2calibrate_likelihood_model(
                    check_target=check_target)
                store_dict.update({"likelihood_home_value_record_average": likelihood_home_value_record_average})
                store_dict.update({"likelihood_away_value_record_average": likelihood_away_value_record_average})
                store_dict.update({"likelihood_all_value_record_average": likelihood_all_value_record_average})
