...
save_csv_name = "td_calibration_lstm_entire_feature_" + str(
    FEATURE_TYPE) + "_" + MODEL_TYPE + "_Iter" + str(ITERATE_NUM) + ".csv"
save_game_csv_name = "td_game_record_lstm_entire_feature_" + str(
    FEATURE_TYPE) + "_" + MODEL_TYPE + "_Iter" + str(ITERATE_NUM) + ".csv"
...
def agg2calibrate_model(check_target):
    ...
    for calibration_dir_game in os.listdir(calibration_store_dir):
        ...
        for file_name in os.listdir(calibration_store_dir + "/" + calibration_dir_game):
            ...
            elif "model_predict_Feature{0}_Iter{1}_Batch{2}_MaxLength{3}".format(str(FEATURE_TYPE), str(ITERATE_NUM),
                                                                                 str(BATCH_SIZE),
                                                                                 str(MAX_LENGTH)) in file_name:
        ...
        model_predict = ((sio.loadmat(model_predict_name))["model_predict_Feature{0}_Iter{1}_Batch{2}_MaxLength{3}".format(str(FEATURE_TYPE), str(ITERATE_NUM),
                                                                                 str(BATCH_SIZE),
                                                                                 str(MAX_LENGTH))]).tolist()
        ...
        for calibrate_name_index in range(0, len(calibrate_names)):
            ...
            if float(check_target.get("GD")) == float(goal_diff) and float(check_target.get("MD")) == float(
                ...
                calibration_value_game_record.append(
                    float(summation_goal_home[calibrate_name_index]) + float(summation_goal_away[calibrate_name_index]))
