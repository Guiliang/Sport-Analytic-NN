...
import unicodedata
import ast
import td_prediction_dp_back_simple_cut_together
import numpy as np
...
MODEL_TYPE = "V3"
BATCH_SIZE = 32
td_prediction_dp_back_simple_cut_together.feature_num = 26
pre_initialize = True
if pre_initialize:
...
if Scale:
    SIMPLE_SAVED_NETWORK_PATH = "/cs/oschulte/Galen/models/saved_NN/Scale-dp-back-cut_saved_entire_together_networks_feature{0}_batch{1}_iterate{2}-NEG_REWARD_GAMMA1_{3}-Sequenced{4}".format(
    ...
    SIMPLE_SAVED_NETWORK_PATH = "/cs/oschulte/Galen/models/saved_NN/dp-back-cut_saved_entire_together_networks_feature{0}_batch{1}_iterate{2}-NEG_REWARD_GAMMA1_{3}-Sequenced{4}".format(
        str(FEATURE_TYPE), str(BATCH_SIZE), str(ITERATE_NUM), MODEL_TYPE, pre_initialize_situation)
...
if MODEL_TYPE == "V3":
    model_nn = td_prediction_dp_back_simple_cut_together.td_prediction_simple_V3()
# elif MODEL_TYPE == "V4":
