...
LOG_DIR = "/cs/oschulte/Galen/models/log_NN/mc_log_" + str(Home_model_or_away_model) + "_train_feature" + str(
    FEATURE_TYPE) + "_batch" + str(
    BATCH_SIZE) + "_iterate" + str(
    ITERATE_NUM) + "-" + str(REWARD_TYPE) + "_" + MODEL_TYPE + TRAIN_or_TEST + "-" + Random_or_Sequenced
SAVED_NETWORK = "/cs/oschulte/Galen/models/saved_NN/mc_saved_" + str(
...
class mc_prediction_simple_V4(object):
    def __init__(self):
        """
        define the neural network
        :return: network output
...
def train_start():
    ...
    if MODEL_TYPE == "V1":
        nn = mc_prediction_simple()
    elif MODEL_TYPE == "V2":
        nn = mc_prediction_simple_V2()
    elif MODEL_TYPE == "V3":
