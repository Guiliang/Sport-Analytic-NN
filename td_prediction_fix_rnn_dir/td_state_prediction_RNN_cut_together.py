...
if SCALE:
    LOG_DIR = save_mother_dir + "/oschulte/Galen/models/log_NN/State-Scale-fix_rnn_cut_together_log_train_feature" + str(
        FEATURE_TYPE) + "_batch" + str(
        BATCH_SIZE) + "_iterate" + str(
        ITERATE_NUM) + "_" + str(MODEL_TYPE)
    SAVED_NETWORK = save_mother_dir + "/oschulte/Galen/models/saved_NN/State-Scale-fix_rnn_cut_together_saved_networks_feature" + str(
        FEATURE_TYPE) + "_batch" + str(
        BATCH_SIZE) + "_iterate" + str(
        ITERATE_NUM) + "_" + str(MODEL_TYPE)
    DATA_STORE = "/cs/oschulte/Galen/Hockey-data-entire/State-RNN-Hockey-Training-All-feature{0}-scale-neg_reward_length-{1}".format(
        str(FEATURE_TYPE), str(TRACE_LENGTH))
else:
    LOG_DIR = save_mother_dir + "/oschulte/Galen/models/log_NN/State-fix_rnn_cut_together_log_train_feature" + str(
        FEATURE_TYPE) + "_batch" + str(
        BATCH_SIZE) + "_iterate" + str(
        ITERATE_NUM) + "_" + str(MODEL_TYPE)
    SAVED_NETWORK = save_mother_dir + "/oschulte/Galen/models/saved_NN/State-fix_rnn_cut_together_saved_networks_feature" + str(
        FEATURE_TYPE) + "_batch" + str(
        BATCH_SIZE) + "_iterate" + str(
        ITERATE_NUM) + "_" + str(MODEL_TYPE)
    DATA_STORE = "/cs/oschulte/Galen/Hockey-data-entire/State-RNN-Hockey-Training-All-feature{0}-neg_reward_length-{1}".format(
        str(FEATURE_TYPE), str(TRACE_LENGTH))
    ...
    while True:
        ...
        for dir_game in DIR_GAMES_ALL:
            ...
            while True:
                ...
                if s_t0_batch[0].shape != (10, 12) or s_t1_batch[0].shape != (10, 12):
                    raise ValueError("Wrong shape of s_t0/1_batch")

                [readout_t1_batch] = sess.run([model.read_out], feed_dict={model.rnn_input: s_t1_batch})
