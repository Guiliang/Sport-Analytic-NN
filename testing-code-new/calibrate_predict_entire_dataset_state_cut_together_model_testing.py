...
import unicodedata
import ast
import td_state_prediction_simple_cut_together_testing
import numpy as np
...
BATCH_SIZE = 32
DATA_SIZE = 100
td_state_prediction_simple_cut_together_testing.feature_num = 12
learning_rate = 1e-5
...
if Scale:
    SIMPLE_SAVED_NETWORK_PATH = "/cs/oschulte/Galen/models/saved_NN/Test{0}-Scale-state-cut_saved_entire_together_networks_feature{1}_batch{2}_iterate{3}_lr{4}-NEG_REWARD_GAMMA1_{5}-Sequenced".format(
        str(DATA_SIZE), str(FEATURE_TYPE), str(BATCH_SIZE), str(ITERATE_NUM), str(learning_rate), MODEL_TYPE)
    calibration_store_dir = "/cs/oschulte/Galen/Hockey-data-entire/TEST{0}-State-Hockey-Training-All-feature{1}-scale-neg_reward".format(
        str(DATA_SIZE), str(FEATURE_TYPE))
else:
    SIMPLE_SAVED_NETWORK_PATH = "/cs/oschulte/Galen/models/saved_NN/Test{0}-state-cut_saved_entire_together_networks_feature{1}_batch{2}_iterate{3}_lr{4}-NEG_REWARD_GAMMA1_{5}-Sequenced".format(
        str(DATA_SIZE), str(FEATURE_TYPE), str(BATCH_SIZE), str(ITERATE_NUM), str(learning_rate), MODEL_TYPE)
    calibration_store_dir = "/cs/oschulte/Galen/Hockey-data-entire/TEST{0}-State-Hockey-Training-All-feature{1}-neg_reward".format(
        str(DATA_SIZE), str(FEATURE_TYPE))
...
if MODEL_TYPE == "V3":
    model_nn = td_state_prediction_simple_cut_together_testing.td_prediction_simple_V3()
# elif MODEL_TYPE == "V4":
...
for calibration_dir_game in os.listdir(calibration_store_dir):
    ...
    data_name = "model_state_cut_together_predict_feature_" + str(
