...
ITERATE_NUM = 25
BATCH_SIZE = 32
calibration_store_dir = "/cs/oschulte/Galen/Hockey-data-entire/td_cut_lstm_calibrate_all_feature_" + str(
...
ISHOME = True
if ISHOME:
    save_csv_name = "td_cut_lstm_home_calibration_entire_feature_" + str(
        FEATURE_TYPE) + "_" + MODEL_TYPE + "_Iter" + str(ITERATE_NUM) + "_batch" + str(
        BATCH_SIZE) + "_sum_2017-6-21.csv"
    save_game_csv_name = "td_cut_lstm_home_game_record_entire_feature_" + str(
        FEATURE_TYPE) + "_" + MODEL_TYPE + "_Iter" + str(ITERATE_NUM) + "_batch" + str(
        BATCH_SIZE) + "_sum_2017-6-21.csv"
    data_home_name = "lstm_model_cut_predict_home_feature_" + str(
        FEATURE_TYPE) + "_" + MODEL_TYPE + "_Iter" + str(ITERATE_NUM) + "_batch" + str(BATCH_SIZE)
else:
    save_csv_name = "td_cut_lstm_away_calibration_entire_feature_" + str(FEATURE_TYPE) + "_" + MODEL_TYPE + "_Iter" + str(
        ITERATE_NUM) + "_batch" + str(BATCH_SIZE) + "_sum_2017-6-21.csv"
    save_game_csv_name = "td_cut_lstm_away_game_record_entire_feature_{0}_{1}_Iter{2}_batch{3}_sum_2017-6-21.csv".format(str(
        FEATURE_TYPE), MODEL_TYPE, str(ITERATE_NUM), str(BATCH_SIZE))
    data_away_name = "lstm_model_cut_predict_away_feature_" + str(
        FEATURE_TYPE) + "_" + MODEL_TYPE + "_Iter" + str(ITERATE_NUM) + "_batch" + str(BATCH_SIZE)
...
def agg2calibrate_model(check_target):
    ...
    for calibration_dir_game in os.listdir(calibration_store_dir):
        ...
        summation_cut_goal = (((sio.loadmat(summation_cut_goal_name))[data_away_name]).tolist())[0]
