...
import unicodedata
import ast
import td_prediction_lstm_testing
import numpy as np
...
SIMPLE_SAVED_NETWORK_PATH = "/cs/oschulte/Galen/models/hybrid_sl_saved_NN/Test{0}-saved_networks_feature{1}_batch{2}_iterate{3}{4}".format(
...
data_name = "model_predict_Feature{0}_Iter{1}_Batch{2}_MaxLength{3}_Type{4}".format(str(FEATURE_TYPE), str(ITERATE_NUM), str(BATCH_SIZE), str(MAX_LENGTH), str(MODEL_TYPE))


def generate_calibration_data():
    sess_nn = tf.InteractiveSession()
    if MODEL_TYPE == "V1":
        model_nn = td_prediction_lstm_testing.td_prediction_lstm()
    elif MODEL_TYPE == "V2":
        model_nn = td_prediction_lstm_testing.td_prediction_lstm_V2()
    elif MODEL_TYPE == "V3":
        model_nn = td_prediction_lstm_testing.td_prediction_lstm_V3()
    else:
