...
Model_train_pre_network_flag = False
if Model_train_pre_network_flag:
    ITERATE_NUM_PRE = 10
    SAVED_NETWORK_pre = "/cs/oschulte/Galen/models/hybrid_sl_saved_NN/Test" + str(
        TEST_LENGTH) + "-saved_networks_feature" + str(
        FEATURE_TYPE) + "_batch" + str(
        BATCH_SIZE) + "_iterate" + str(
        ITERATE_NUM_PRE) + MODEL_TYPE

SPORT = "NHL"
REWARD_TYPE = "NEG_REWARD_GAMMA1_V3"
DATA_STORE = "/cs/oschulte/Galen/Hockey-data-entire/Test" + str(
    TEST_LENGTH) + "-Hybrid-RNN-Hockey-Training-All-feature" + str(
    FEATURE_TYPE) + "-scale-neg_reward_length-dynamic"
LOG_DIR = "/cs/oschulte/Galen/models/hybrid_sl_log_NN/Test" + str(TEST_LENGTH) + "-log_train_feature" + str(
    FEATURE_TYPE) + "_batch" + str(
    BATCH_SIZE) + "_iterate" + str(
    ITERATE_NUM) + MODEL_TYPE
SAVED_NETWORK = "/cs/oschulte/Galen/models/hybrid_sl_saved_NN/Test" + str(
    TEST_LENGTH) + "-saved_networks_feature" + str(
    FEATURE_TYPE) + "_batch" + str(
    BATCH_SIZE) + "_iterate" + str(
    ITERATE_NUM) + MODEL_TYPE
...
def train_network(sess, model, print_parameters=False):
    ...
    if model_train_continue:
        if Model_train_pre_network_flag:
            checkpoint = tf.train.get_checkpoint_state(SAVED_NETWORK_pre)
        else:
