...
import unicodedata
import ast
import mc_together_prediction_simple_cut_testing
import numpy as np
...
BATCH_SIZE = 32
DATA_SIZE = 100
mc_together_prediction_simple_cut_testing.feature_num = 26

if Scale:
    SIMPLE_SAVED_NETWORK_PATH = "/cs/oschulte/Galen/models/saved_NN/mc-Scale-Test{0}-cut_saved_entire_together_networks_feature{1}_batch{2}_iterate{3}-NEG_REWARD_GAMMA1_{4}-Sequenced".format(
    ...
    SIMPLE_SAVED_NETWORK_PATH = "/cs/oschulte/Galen/models/saved_NN/mc-Test{0}-cut_saved_entire_together_networks_feature{1}_batch{2}_iterate{3}-NEG_REWARD_GAMMA1_{4}-Sequenced".format(
        str(DATA_SIZE), str(FEATURE_TYPE), str(BATCH_SIZE), str(ITERATE_NUM), MODEL_TYPE)
...
if MODEL_TYPE == "V3":
    model_nn = mc_together_prediction_simple_cut_testing.td_prediction_simple_V3()
# elif MODEL_TYPE == "V4":
...
#     model_nn = td_prediction_simple_cut_together_testing.td_prediction_simple_V7()
    ...
    model_nn = mc_together_prediction_simple_cut_testing.td_prediction_simple_V8()
else:
