...
import unicodedata
import ast
import td_prediction_back_simple_cut_testing
import numpy as np
...
BATCH_SIZE = 32
DATA_SIZE = 100
td_prediction_back_simple_cut_testing.feature_num = 26
Scale = True

if Scale:
    SIMPLE_SAVED_NETWORK_PATH = "/cs/oschulte/Galen/models/saved_NN/Scale-Test{0}-back-cut_saved_entire__networks_feature{1}_batch{2}_iterate{3}-NEG_REWARD_GAMMA1_{4}-Sequenced".format(
    ...
    SIMPLE_SAVED_NETWORK_PATH = "/cs/oschulte/Galen/models/saved_NN/Test{0}-back-cut_saved_entire__networks_feature{1}_batch{2}_iterate{3}-NEG_REWARD_GAMMA1_{4}-Sequenced".format(
        str(DATA_SIZE), str(FEATURE_TYPE), str(BATCH_SIZE), str(ITERATE_NUM), MODEL_TYPE)
...
if MODEL_TYPE == "V1":
    model_nn = td_prediction_back_simple_cut_testing.td_prediction_simple()
elif MODEL_TYPE == "V2":
    model_nn = td_prediction_back_simple_cut_testing.td_prediction_simple_V2()
elif MODEL_TYPE == "V3":
    model_nn = td_prediction_back_simple_cut_testing.td_prediction_simple_V3()
elif MODEL_TYPE == "V4":
    model_nn = td_prediction_back_simple_cut_testing.td_prediction_simple_V4()
elif MODEL_TYPE == "V5":
    model_nn = td_prediction_back_simple_cut_testing.td_prediction_simple_V5()
elif MODEL_TYPE == "V6":
    model_nn = td_prediction_back_simple_cut_testing.td_prediction_simple_V6()
elif MODEL_TYPE == "V7":
    model_nn = td_prediction_back_simple_cut_testing.td_prediction_simple_V7()
else:
...
for calibration_dir_game in os.listdir(calibration_store_dir):
    ...
    data_name = "model_cut_back_predict_feature_" + str(
