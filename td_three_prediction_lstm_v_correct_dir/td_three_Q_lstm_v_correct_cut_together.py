...
LOG_DIR = save_mother_dir + "/oschulte/Galen/models/hybrid_sl_log_NN/Scale-three-Q-cut_together_log_train_feature" + str(
    FEATURE_TYPE) + "_batch" + str(
    BATCH_SIZE) + "_iterate" + str(
    ITERATE_NUM) + "_lr" + str(
    learning_rate) + "_" + str(MODEL_TYPE) + if_correct_velocity
SAVED_NETWORK = save_mother_dir + "/oschulte/Galen/models/hybrid_sl_saved_NN/Scale-three-Q-cut_together_saved_networks_feature" + str(
    FEATURE_TYPE) + "_batch" + str(
    BATCH_SIZE) + "_iterate" + str(
    ITERATE_NUM) + "_lr" + str(
    learning_rate) + "_" + str(MODEL_TYPE) + if_correct_velocity
...
class td_prediction_lstm_V3:
    def __init__(self, rnn_type='bp_last_step'):
        ...
        self.action = tf.placeholder("float", [None, ACTION_NUMBER * 3])
...
def get_together_training_batch(s_t0, state_input, action, reward, train_number, train_len, state_trace_length):
    """
    ...
    while current_batch_length < BATCH_SIZE:
        ...
        if train_number + 1 == train_len:
            ...
            if r_t0 == [float(0)]:
                ...
                batch_return.append((s_t0, a_t0, s_t1, a_t1, r_t0_combine, s_length_t0, s_length_t1, 0, 0))
                ...
                batch_return.append((s_t1, a_t1, s_t1, a_t1, r_t1_combine, s_length_t1, s_length_t1, 1, 0))
        ...
        if r_t0 != [float(0)]:
            ...
            if r_t0 == [float(-1)]:
                r_t0_combine = [float(0), float(1), float(0)]
                batch_return.append((s_t0, a_t0, s_t1, a_t1, r_t0_combine, s_length_t0, s_length_t1, 0, 1))
            elif r_t0 == [float(1)]:
                r_t0_combine = [float(1), float(0), float(0)]
                batch_return.append((s_t0, a_t0, s_t1, a_t1, r_t0_combine, s_length_t0, s_length_t1, 0, 1))
            else:
